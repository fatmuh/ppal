...
<body>
    <div id="imgDiv" style="width: 1003px; height: 661px">
        ...
        <p class="foto">
            @if (Str::startsWith($data->foto, 'foto/'))
                <!-- Jika $data->foto adalah path -->
                <img src="/get-image?image={{ $data->foto }}" width="199px" height="301px">
            @else
                <!-- Jika $data->foto adalah base64 -->
                <img src="data:image/jpeg;base64,{{ $data->foto }}" width="199px" height="301px">
            @endif
        </p>
        <p class="bar128"><img
                src='{{ url('barcode.php?codetype=code128&sizefactor=1&size=50&text=' . $data->no_kta) }}' /></p>
        <p class="tanggal"><b>{{ \Carbon\Carbon::now()->locale('id_ID')->isoFormat('DD MMMM YYYY') }}</b></p>
        <p class="ttd"><b>
                @if (Str::startsWith($data->ttd, 'ttd/'))
                    <!-- Jika $data->ttd adalah path -->
                    <img src="/get-image?image={{ $data->ttd }}">
                @else
                    <!-- Jika $data->ttd adalah base64 -->
                    <img src="data:image/jpeg;base64,{{ $data->ttd }}">
                @endif
            </b></p>
    </div>
